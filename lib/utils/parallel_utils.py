# copyright@https://github.com/dendenxu/longvolcap/blob/101c00b/easyvolcap/utils/parallel_utils.py
    ...
    if not sequential:
        ...
        if not async_return:
            for async_result in tqdm(
                asyncs, back=3, desc=desc, disable=not print_progress
            ):  # log previous frame
        ...
        length = get_length(args, kwargs)
        for i in tqdm(
            range(length), back=3, desc=desc, disable=not print_progress
        ):  # log previous frame
            action_args, action_kwargs = get_action_args(length, args, kwargs, i)
