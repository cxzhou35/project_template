# copyright@https://github.com/dendenxu/longvolcap/blob/101c00b/easyvolcap/utils/console_utils.py
...
try:
    for package_name, import_name in git_essential_packages.items():
        __import__(import_name)
except ImportError as e:
    print(f'{Colors.YELLOW}{Colors.BOLD}Missing package: {Colors.RED}{Colors.BOLD}{e}{Colors.YELLOW}{Colors.BOLD}, trying to hot install using pip...{Colors.END}')
    import sys
...
from tqdm.rich import RateColumn, FractionColumn, tqdm_rich
...
from lib.utils.base_utils import DoNothing, dotdict, default_dotdict
...
force_terminal = (
    bool(os.environ["EASYVOLCAP_CONSOLE_FORCE_TERMINAL"])
...
from pygments.token import Token, Number, String, Comment, Keyword, Operator
...
def read(cmd: str, *args, **kwargs):
    ...
    return run(cmd, invocation=get_output, skip_failed=True, *args, **kwargs)
...
class ColoredLogger:
    ...
    def info(self, things: str):
        from easyvolcap.utils.dist_utils import is_main_process

        if not is_main_process():
            return
        log(self.prefix + str(things) + self.postfix)

    def debug(self, things: str):
        from easyvolcap.utils.dist_utils import is_main_process

        if not is_main_process():
            return
        log(green_slim(self.prefix + str(things) + self.postfix))

    def warn(self, things: str):
        from easyvolcap.utils.dist_utils import is_main_process

        if not is_main_process():
            return
        log(yellow(self.prefix + str(things) + self.postfix))

    def error(self, things: str):
        from easyvolcap.utils.dist_utils import is_main_process

        if not is_main_process():
            return
        log(red(self.prefix + str(things) + self.postfix))


class DualOutput(io.TextIOWrapper):
    def __init__(self, file_path, mode="w", encoding="utf-8"):
        ...
        # Initialize the TextIOWrapper
        super().__init__(self.file.buffer, encoding=encoding)
    ...
    def get_logger(name: str):
        if name not in logging.mapping:
            logging.mapping[name] = ColoredLogger(
                prefix=green(f"[{name}] ") if name else ""
            )
...
class Timer:
    def __init__(
        ...
        self.start_time = (
            time.perf_counter()
        )  # manually record another start time incase timer is disabled during initialization
    ...
    def record(self, event: str = "", log_interval: float = -1):
        ...
        if (
            curr - last
        ) > log_interval and log_interval > 0:  # if this is true, will never have printed
