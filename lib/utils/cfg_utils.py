...
def parse_cfg(cfg, args):
    ...
    if os.path.exists(cfg.model_path) and cfg.mode == "train":
        log("Model path already exists, this would override original model")
        log(f"model_path: {cfg.model_path}")
...
def save_cfg(cfg, model_dir, epoch=0):
    ...
    log(f"Save input config to {cfg_path}")
